...
L['elite_loot_385'] = '{note:Из этого редкого монстра можно получить добычу более высокого уровня!}'
L['gem_cluster_note'] = 'Эти предметы станут доступны с 21-го уровня известности с {faction:Драконьей Экспедицией}, их можно найти в {object:Сумках разведчиков экспедиции} и в {object:Потревоженной почве}.'
...
L['fragment_requirement_note'] = ns.color.Red('Прежде чем вы сможете собирать разбросанные предметы, вам нужно спросить {npc:193915} в {location:посольстве Крылатого Приюта} что она здесь делает, ее можно найти у статуи дракона.')
...
L['snowclaw_cub_note_start'] = 'Вы должны выполнить цепочку заданий {quest:67094}, предложенную {npc:192522} в {sanctuary:Вальдраккене} чтобы получить звание {title:почетная дриада}.\n\nСоберите следующие предметы:'
...
L['elder_poa_note'] = 'Обменяйте {item:200071} на репутацию с {faction:Искарскими клыкаррами}.'
...
L['picante_pomfruit_cake_note'] = '{item:200904} доступен не каждый день, поэтому проверяйте {sanctuary:Рубиновый Пир} ежедневно. Пока вы там, не забудьте попробовать 3 доступных блюда, чтобы выполнить {achievement:16556}.'
...
L['lillian_brightmoon_note'] = 'Обменять {item:201412} на репутацию с |cFFFFFD00Драконьей экспедицией|r.'
