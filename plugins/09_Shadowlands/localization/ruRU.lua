local ADDON_NAME, ns = ...
local L = ns.NewLocale('ruRU')
if not L then return end
-- Russian translation by ZamestoTV

-------------------------------------------------------------------------------
---------------------------------- COVENANTS ----------------------------------
-------------------------------------------------------------------------------

L['covenant_required'] = 'Требуется член ковенанта %s.'
L['anima_channeled'] = 'анима направленая на %s.'

-------------------------------------------------------------------------------
--------------------------------- SHADOWLANDS ---------------------------------
-------------------------------------------------------------------------------

L['squirrels_note'] = 'Вы должны использовать эмоцию /любовь на зверьках, а не на боевых питомцах.'
L['options_icons_squirrels'] = '{achievement:14731}'
L['options_icons_squirrels_desc'] = 'Места появления зверьков для {achievement:14731}'

L['options_icons_safari'] = '{achievement:14867}'
L['options_icons_safari_desc'] = nil

-------------------------------------------------------------------------------
--------------------------------- ARDENWEALD ----------------------------------
-------------------------------------------------------------------------------

L['deifir_note'] = 'Катайся на редком монстре по кругу и используй {spell:319566} и {spell:319575} чтобы замедлить и оглушить его.'
L['faeflayer_note'] = 'В маленькой пещере, скрытой за водопадом.'
L['gormbore_note'] = 'Убейте {npc:165420} над грохочущей землей, чтобы вызвать редкого монстра.'
L['gormtamer_tizo_note'] = 'Убивайте Острохвойных заклинателей, пока не появится {npc:164110}'
L['humongozz_note'] = 'Посадите {item:175247} во влажном суглинке, чтобы создать {npc:164122}. Гриб выпадает с многочисленных мобов в зоне.'
L['lehgo_note'] = 'Уничтожьте подрагивающую кладку гормов и убивайте {npc:171827}, пока он не появится. В пещере (вход на юго-восток Пыльных Нор).'
L['macabre_note'] = [[
Появляется в нескольких местах. Чтобы призвать, встаньте в Кольцо Таинственных Грибов с двумя другими игроками и танцуйте друг с другом.

• Игрок 1 танцует с Игроком 2
• Игрок 2 танцует с Игроком 3
• Игрок 3 танцует с Игроком 1
]]
L['mymaen_note'] = 'Убивайте гнилошипов в области, пока он не проявит эмоции и не появится.'
L['rainbowhorn_note'] = [[
Найдите и нажмите на |cffffff00Большой рог рунического оленя|r чтобы вызвать редкого монстра. Рог может появиться в нескольких местах Арденвельда.

Он всегда будет появляться к северу от Тирна Ваал, поэтому установите там свой {item:6948} и следите за оповещением в локации.

|cffff5400В|r|cffffaa00К|r|cffffff00У|r|cffaaff00С|r|cff54ff00Ы|r  |cff0055ffР|r|cff0000ffА|r|cff5400ffД|r|cffaa00ffУ|r|cffff00ffГ|r|cffff00aaИ|r|cffff0000!|r
]]
L['rootwrithe_note'] = 'Тыкай по {npc:167928} пока не появится редкий монстр.'
L['rotbriar_note'] = 'Поговорите с {npc:171684}, чтобы вызвать редкого монстра после некоторого диалога.'
L['slumbering_note'] = 'Не заходите в туман, он оглушит вас и перенесет. Используйте сигнальную ракету или питомца с AOE, чтобы выбить его из тумана.'
L['skuld_vit_note'] = 'В пещере, заблокированной барьером. Игрок из Ночного Народца должен использовать {spell:310143} чтобы войти в пещеру. Как только монстра атакуют, барьер исчезнет.'
L['valfir_note'] = 'Нажмите на свет анимасемени и на полпути используйте {spell:338045} чтобы убрать его бафф {spell:338038}'
L['wrigglemortis_note'] = 'Потяните за {npc:164179} чтобы призвать редкого монстра.'

L['night_mare_note'] = [[
Отправляйтесь в Тирна Скитт и следуйте по пути из корней на северо-западном утесе к сломанной тележке. Там вы можете добыть {item:181243} на земле.

Отнесите этот предмет к {npc:165704} в Низину Мерцающих Каскадов. Она даст вам {item:181242} в обмен на 10 {item:173204}. |cFFFF8C00Если ее нет, вы должны выполнить |cFFFFFD00Проблемы в загоне гормлингов|r и |cFFFFFD00Хитроумные спригганы|r линии заданий.|r

Затем поговорите с {npc:160262} в самом сердце леса, чтобы обменять {item:181242} на {item:178675}. Используйте этот предмет, чтобы получить бафф {spell:327083}, позволяющий увидеть {npc:168135}.
]]

L['star_lake'] = 'Амфитеатр Звездного озера'
L['star_lake_note'] = [[
Поговорите с постановщиком {npc:171743}, чтобы запустить одну из сцен. Сцена меняется каждый день.

Примите участие во всех семи сценах, чтобы разблокировать {item:180748} у {npc:163714}.
]]

L['cache_of_the_moon'] = 'Соберите пять недостающих инструментов {npc:171360} в саду ночи и объедините их, чтобы создать {item:180753}. Верните ее набор инструментов, и она наложит на вас {spell:334353}, что позволит вам увидеть сокровище.'
L['cache_of_the_night'] = 'Соберите {item:180656}, {item:180654} и {item:180655} которые добываются со всей локации и объедините их, чтобы создать {item:180652}.'
L['darkreach_supplies'] = 'Прыгайте на {npc:169995} и летите на юго-запад, в выдолбленный ствол прямо к припасам темного края.'
L['desiccated_moth'] = 'Перейти на {npc:169997} к северо-западу от дерева, чтобы подняться на ветку. Сжечь {item:180784} в курильнице, чтобы собрать сокровище.'
L['dreamsong_heart'] = 'Используйте {npc:169997} под деревом, чтобы попасть в дерево.'
L['elusive_faerie_cache'] = 'Примените {spell:333923} в северо-восточном углу Сумеречной Рощи и используйте ее, чтобы обыскать сундук.'
L['enchanted_dreamcatcher'] = 'Свисает с верхушки корней. Легче всего прыгать с западной стороны.'
L['faerie_trove'] = 'Расположен под платформой.'
L['harmonic_chest'] = 'Требуется два человека. Один человек играет на арфе, а другой играет на барабане, чтобы открыть сундук.'
L['hearty_dragon_plume'] = 'Нажмите {spell:333554} на вершине близлежащих водопадов и используйте его, чтобы спуститься на ветку.'
L['old_ardeite_note'] = 'Убейте {npc:160747} и {npc:160748} в Мерцающей рощи на юго-востоке для {item:174042}. Используйте этот предмет, чтобы подлететь к редкому монстру и взять его в цель.'
L['swollen_anima_seed'] = 'Большое семя, сидящее внутри ствола дерева.'

L['playful_vulpin_note'] = [[
Найдите и используйте правильную эмоцию на {npc:171206} пять раз, чтобы получить питомца.

• начинает с любопытством копать = |cFFFF8C00/любопытство|r
• ходит вокруг, не желая сидеть смирно = |cFFFF8C00/сесть|r
• поет совсем один = |cFFFF8C00/песня|r
• танцует от радости = |cFFFF8C00/танец|r
• сидит один и грустит = |cFFFF8C00/гладить|r
]]

L['tame_gladerunner'] = 'Мерцающий туманный бегун'
L['tame_gladerunner_note'] = [[
Прочтите «История Чащобы» и следуйте по дорожке по синим лампам, пока не дойдете до {npc:171767}. Убейте его и возьмите {npc:171699}.

Если вы выберете неправильный путь и {npc:171699} исчезнет, когда вы дойдете до конца, вернитесь к началу и прочтите «История Чащобы» еще раз, прежде чем пытаться снова. Если {npc:171767} там нет, вам придется подождать, пока он появится.
]]

L['faryl_note'] = 'Пусть защиту Арденвельда возглавят существа в небе.'
L['glitterdust_note'] = 'Существа Арденвельда могут выглядеть послушными, но они будут защищать свою территорию с силой и мужеством самых могущественных чемпионов из когда-либо известных. У вас есть то, что нужно?'

L['lost_book_note'] = 'Верните эту потерянную книгу {npc:165867} в Рощу Памяти.'
L['options_icons_faerie_tales'] = '{achievement:14788}'
L['options_icons_faerie_tales_desc'] = 'Показать местонахождение потерянных книг для достижения {achievement:14788}.'

L['options_icons_wild_hunting'] = '{achievement:14779}'
L['options_icons_wild_hunting_desc'] = 'Животные в Арденвельде для {achievement:14779}'

L['options_icons_wildseed_spirits'] = 'Духи из семян'
L['options_icons_wildseed_spirits_desc'] = 'Награды от духов из семян'

L['divine_martial_spirit'] = 'Божественный воинственный дух'
L['divine_dutiful_spirit'] = 'Божественный дух служения'
L['divine_prideful_spirit'] = 'Божественный дух гордыни'
L['divine_untamed_spirit'] = 'Божественный неукрощенный дух'

L['martial_spirit_label'] = '{item:178874}'
L['dutiful_spirit_label'] = '{item:178881}'
L['prideful_spirit_label'] = '{item:178882}'
L['untamed_spirit_label'] = '{item:177698}'

L['0x_wildseed_root_grain'] = '0x {item:176832}'
L['1x_wildseed_root_grain'] = '1x {item:176832}'
L['2x_wildseed_root_grain'] = '2x or 3x {item:176832}'
L['4x_wildseed_root_grain'] = '4x {item:176832}'

L['soulshape_cat_note'] = [[
Возьмите в цель {npc:181694} и используйте эмоцию /утешить
Может появиться в центре вершины 6 Великих Деревьев вокруг |cFFFFFF00Арденвельда|r:
• |cFFFFFF00Роща Песни Сна|r
• |cFFFFFF00Низина Мерцающих Каскадов|r
• |cFFFFFF00Тирна Ваал|r
• |cFFFFFF00Спящая лощина|r
• |cFFFFFF00Сердцевинная роща|r
• |cFFFFFF00Урочищий Коготь|r
]]
L['soulshape_corgi_note'] = [[
Возьмите в цель {npc:174608} и используйте эмоцию /погладить
Затем сразу после разговора с корги идите к {npc:181582}
]]
L['soulshape_well_fed_cat_note'] = [[
1. Заберите {item:187811} в |cffffff00Темном Приюте|r
2. Возьмите в цель {npc:182093} и используйте эмоцию /мау
3. |cFFFF8C00Ма'о жадно мяукает на тебя.|r
4. Возьмите в цель {npc:182093} и используйте {item:187811}
При разговоре с {npc:181582} сразу станет доступна опция Душа кошки (Сытой).
]]

-------------------------------------------------------------------------------
----------------------------------- BASTION -----------------------------------
-------------------------------------------------------------------------------

L['aegeon_note'] = 'Убивайте врагов в окрестностях, пока Каратель Эгеон не появится в качестве подкрепления.'
L['ascended_council_note'] = 'Вместе с четырьмя другими игроками нажмите одновременно пять колоколов. После этого отправляйтесь в горнило претендента, чтобы победить Вознесенный Совет и открыть сундук.'
L['aspirant_eolis_note'] = 'Найти {item:180613}, который можно найти поблизости и использовать рядом с НПС.'
L['baedos_note'] = 'Несите бочки с ферментированным чистоплодом из окрестностей до {npc:161536}, пока он не активируется.'
L['basilofos_note'] = 'Двигайтесь вокруг камня, пока над вашей головой не появится фиолетовый маркер. Остановитесь и подождите, пока появятся четыре эмоции, после этого появится монстр.'
L['beasts_of_bastion'] = 'Звери Бастиона'
L['beasts_of_bastion_note'] = 'Поговорите с {npc:161441}, чтобы призвать одного из четырех зверей.'
L['bookkeeper_mnemis_note'] = 'Имеет шанс появиться вместо {npc:166867}.'
L['cloudfeather_patriarch_note'] = 'Убивайте {npc:158110} в этом районе до появления редкого монстра.'
L['collector_astor_note'] = 'Прочтите все шесть глав «Наследие Милосерды», разбросанные по комнате, затем поговорите с {npc:157979}, чтобы получить {spell:333779}. Найдите спрятанный {item:180569} в окрестностях и верните его, чтобы призвать редкого монстра.'
L['corrupted_clawguard_note'] = 'Возьмите {item:180651} на холме в лагере кузнецов или в комнате с редким монстром и используйте его для ремонта {npc:171300}.'
L['dark_watcher_note'] = 'Виден только мертвым. Поговори с ней, и она применит {spell:332830} на вас перед атакой.'
L['demi_hoarder_note'] = 'Начинается с 99 стаков {spell:333874}, уменьшения получаемого урона. Стаки медленно снимаются с повреждениями. Редкий монстр пойдет по пути и исчезнет, если он достигнет его конца.'
L['dionae_note'] = 'Когда она станет неуязвимой, нажмите на четыре {npc:163747}, чтобы сломать ее щит.'
L['herculon_note'] = [[
Соберите {item:172451} и используйте их, чтобы дать {npc:158659} стаки {spell:343531}. При 10 стаках он активируется.

Частицы можно собрать из близлежащих Канистр анимы, расположенных в комнате или снаружи.

|cFFFF8C00Нельзя активировать во время локального задания {wq:Нападение на Преддверие Вечности}.|r
]]
L['reekmonger_note'] = 'Убивайте врагов в Храме отваги, пока не появится {npc:171327} сигналом появления будет эмоция.'
L['repair_note'] = 'Вместе с двумя другими игроками нажмите древнее благовоние, чтобы призвать монстра.'
L['sotiros_orstus_note'] = 'Нажмите на Черный колокол, чтобы вызвать редкого монстра.'
L['sundancer_note'] = 'Нажмите на статую, чтобы получить бафф {spell:332309}, затем используйте {item:180445}, чтобы лететь к редкому монстру и получить средство передвижения.'
L['swelling_tear_note'] = 'Нажмите на {npc:171012}, чтобы вызвать одного из трех редких монстров. Разломы могут появляться в нескольких местах в локации.'
L['unstable_memory_note'] = 'Может появиться при наличии {npc:171018}. Просто подводите {npc:171018} к другим, чтобы получить 10 стаков {spell:333558}, превратив его в редкого монстра.'
L['wingflayer_note'] = 'Чтобы призвать, нажмите Рог Отваги на ближайшем столе (на юго-востоке, вверх по лестнице).'

L['broken_flute'] = 'Убивайте поблизости {npc:170009}, пока они не уронят {item:180536}, затем воспользуйтесь инструментами, чтобы починить его.'
L['cloudwalkers_coffer'] = 'Сундук Облачного Странника'
L['cloudwalkers_coffer_note'] = 'Используйте большие фиолетовые цветы, чтобы подпрыгнуть на платформу.'
L['experimental_construct_part'] = 'Найдите ближайший {item:180534} и используйте его, чтобы починить деталь. У анимы есть несколько мест появления.'
L['larion_harness'] = 'Находится внутри Звериного Зала.'
L['memorial_offering'] = 'Купите {item:180788} у {npc:171526} в Обители героев. Поместите его в поднос с напитками возле сундука, чтобы получить {item:180797}.'
L['scroll_of_aeons'] = 'Добудьте 2 {item:173973} в центре и поместите их на близлежащие подносы, чтобы найти сокровище.'
L['vesper_of_silver_wind'] = 'Колокол Серебряного Ветра'
L['vesper_of_silver_wind_note'] = 'Завершите достижение {achievement:14339}, а затем поговорите с {npc:171732} у входа в Шпили Перерождения, чтобы выковать {item:180858}.'

L['gift_of_agthia'] = 'Нажмите на зажженный факел возле сломанного моста на северо-западе и переносите {spell:333320} от факела к факелу, пока не дойдете до сундука. Зажигание последнего факела даст вам {spell:333063}.'
L['gift_of_chyrus'] = 'Встать на колени перед сундуком, чтобы получить {spell:333045}.'
L['gift_of_devos'] = [[
К юго-западу от сундука находится факел, где вы можете поднять {spell:333912}. Садясь, вступая в бой или получая урон, пламя падет. Вы должны вернуться к сундуку и поместить пламя в Жаровню Преданности, чтобы получить {spell:333070}.

Прежде чем поднять пламя, убейте всех монстров перед сундуком. Взяв пламя, нажмите на любого {npc:156571} по пути, чтобы активировать дебафф {spell:335012}, увеличивающий скорость передвижения.
]]
L['gift_of_thenios'] = [[
Позади сундука находится летная площадка под названием «Путь мудрости». Это приводит к последовательности платформ с разными благовониями, с которыми вы можете общаться:

• Благовония знаний
• Благовония терпения
• Благовония озарения
• Благовония правого суда

Общайтесь с ними в следующем порядке: Терпение => Знание => Озарение. На платформе Правого суда появится шар под названием «Путь правого суда».

Сфера перенесет вас к Благовонию правого суда. Пообщайтесь с ним, и последняя панель полета предоставит вам {spell:333068}, чтобы открыть сундук.
]]
L['gift_of_vesiphone'] = 'Позвоните в один из колокольчиков, чтобы вызвать {npc:170849}, и убейте его, чтобы получить дебафф {spell:333239}. Падающая вода напротив сундука снимет этот дебафф и даст вам {spell:332785}.'

L['count_your_blessings_note'] = 'Поместите {item:178915} в чашу для подношений, чтобы получить благословение.'
L['options_icons_blessings'] = '{achievement:14767}'
L['options_icons_blessings_desc'] = 'Показать места награждения за достижение {achievement:14767}.'

L['vesper_of_courage'] = 'Колокол Отваги'
L['vesper_of_humility'] = 'Колокол Смирения'
L['vesper_of_loyalty'] = 'Колокол Верности'
L['vesper_of_purity'] = 'Колокол Чистоты'
L['vesper_of_wisdom'] = 'Колокол Мудрости'
L['vespers_ascended_note'] = 'Активируйте этот колокол одновременно с четырьмя другими колоколами, чтобы призвать {npc:170899} в Горнило претендентов.'
L['options_icons_vespers'] = '{achievement:14734}'
L['options_icons_vespers_desc'] = 'Показать расположение колоколов для достижения {achievement:14734}.'

L['anima_shard'] = 'Пропавшие осколки анимы'
L['anima_shard_61225'] = 'На нижней площадке под мостомe.'
L['anima_shard_61235'] = 'На уступе над водопадом.'
L['anima_shard_61236'] = 'На вершине арки на полпути к основному строению.'
L['anima_shard_61237'] = 'На выступе прямо над водой.'
L['anima_shard_61238'] = 'В воде под мостиком.'
L['anima_shard_61239'] = 'На вершине тонкой каменной колонны.'
L['anima_shard_61241'] = 'Над входом в Зал Первого Созерцания.'
L['anima_shard_61244'] = 'На скале в стороне обрыва.'
L['anima_shard_61245'] = 'На скале над небольшим водопадом.'
L['anima_shard_61247'] = 'На стене над небольшой водопроводной арматурой.'
L['anima_shard_61249'] = 'Спрятан за каменной колонной на верхнем уровне Вершины Чистоты.'
L['anima_shard_61250'] = 'За лестницей.'
L['anima_shard_61251'] = 'Под маленьким колоколом.'
L['anima_shard_61253'] = 'На вершине упавшей каменной арки.'
L['anima_shard_61254'] = 'Сверху небольшой деревянной конструкции.'
L['anima_shard_61257'] = 'На небольшом выступе прямо под {npc:162523}.'
L['anima_shard_61258'] = 'На небольшом выступе на нижней стороне Обители Героев.'
L['anima_shard_61260'] = 'На земле под платформой.'
L['anima_shard_61261'] = 'На выступе над пещерой {npc:163460}.'
L['anima_shard_61263'] = 'На вершине каменного столба.'
L['anima_shard_61264'] = 'Поверх наклонной конструкции.'
L['anima_shard_61270'] = 'У подножия дерева.'
L['anima_shard_61271'] = 'В книжном шкафу на верхней площадке.'
L['anima_shard_61273'] = 'На уступе прямо под выступающей скалой.'
L['anima_shard_61274'] = 'Спрятан под платформой.'
L['anima_shard_61275'] = 'В Зале Зверей за бочками.'
L['anima_shard_61277'] = 'На вершине тонкой каменной колонны.'
L['anima_shard_61278'] = 'Под мостом на скале.'
L['anima_shard_61279'] = 'На вершине тонкой каменной колонны.'
L['anima_shard_61280'] = 'На углу стола.'
L['anima_shard_61281'] = 'На уступе над сокровищем Подношения у мемориала.'
L['anima_shard_61282'] = 'На уступе ниже вершины утеса. Прыгайте на камень рядом с ним, иначе вас перенесут обратно.'
L['anima_shard_61283'] = 'В пещере под часовней Мири, за бочками.'
L['anima_shard_61284'] = 'На уступе под скальным навесом тропинка на юг.'
L['anima_shard_61285'] = 'В конце небольшого уступа скалы.'
L['anima_shard_61286'] = 'На выступе с видом на тропу.'
L['anima_shard_61287'] = 'На уступе над небольшим водопадом.'
L['anima_shard_61288'] = 'На кончике небольшого уступа.'
L['anima_shard_61289'] = 'На крыше беседки.'
L['anima_shard_61290'] = 'В конце узкого уступа скалы.'
L['anima_shard_61291'] = 'На дне пруда у ног статуи.'
L['anima_shard_61292'] = 'На вершине каменной арки.'
L['anima_shard_61293'] = 'На вершине тонкой каменной колонны на нижнем уровне.'
L['anima_shard_61294'] = 'Спрятан за бочками.'
L['anima_shard_61295'] = 'За {npc:156889} на книжной полке.'
L['anima_shard_61296'] = 'За большим упавшим колоколом.\n\n' .. ns.color.Red('Внутри подземелья Смертельная тризна.')
L['anima_shard_61297'] = 'За каменным столбом.\n\n' .. ns.color.Red('Внутри подземелья Смертельная тризна.')
L['anima_shard_61298'] = 'За шезлонгом.'
L['anima_shard_61299'] = 'Скрытый за большим факелом.'
L['anima_shard_61300'] = 'На выступе, нависшем над центральной купелью.'
L['anima_shard_spires'] = 'Три осколка находятся в подземелье Шпили Перерождения.'
L['options_icons_anima_shard'] = '{achievement:14339}'
L['options_icons_anima_shard_desc'] = 'Отображение местоположения всех 50 потерянных осколков анимы для достижения {achievement:14339}.'

L['hymn_note'] = 'Найдите гимны в каждом из храмов и получите их баффы, чтобы заработать достижение.'
L['options_icons_hymns'] = '{achievement:14768}'
L['options_icons_hymns_desc'] = 'Отображение расположения гимнов для достижения {achievement:14768}.'

L['stratios_note'] = 'Даже самые маленькие битвы нужно вести с честью и осторожностью. Представьте свою команду, когда будете готовы.'
L['thenia_note'] = 'Вот такие великолепные открытые равнины. Славное место для битв. Вы готовы?'
L['zolla_note'] = 'Мы очень серьезно относимся к нашей защите. Независимо от того, малые они или большие, мы полностью привержены поддержанию и обучению ресурсов, которые делают Бастион сильным.'

L['soulshape_otter_soul'] = 'Возьмите в цель {npc:181682} и используйте эмоцию /обнять'

-------------------------------------------------------------------------------
----------------------------------- KORTHIA -----------------------------------
-------------------------------------------------------------------------------

L['carriage_crusher_note'] = 'Следуйте за {npc:180182} в Утробу и защищайте его, пока {npc:180246} не атакуют.'
L['chamber_note'] = 'Используйте {item:186718} из {npc:178257} на древнем телепорте. Используйте отремонтированный древний телепорт, чтобы получить доступ к комнате.'
L['consumption_note'] = [[
Этот редкий монстр не будет давать добычу до тех пор, пока он не поглотит 40 {npc:179758}, чтобы превратиться в свою продвинутую форму, с синим оттенком (редкий).

Из этого редкого монстра будут выпадать дополнительные фрагменты исследований после того, как он *поглотит* еще 40 {npc:179758}, и примет зеленую форму (элитный редкий).

|cFFFF0000Редкий монстр не будет порождать {npc:179758} находясь в бою.|r
]]
L['darkmaul_note'] = 'Соберите {item:187153} из инвазивных грибов и скормите их {npc:180063}. Вы должны пройти событие 10 раз, чтобы получить средство передвижение.'
L['dislodged_nest_note'] = 'Нажмите на ближайшего Токсичного мотылька, чтобы получить {spell:355181}. Используйте дополнительную кнопку действия {spell:355131} на {npc:178547}, чтобы получить контроль и врезаться в дерево, на котором находится гнездо.'
L['escaped_wilderling_note'] = 'Нажмите на {npc:180014}, чтобы начать приручение.'
L['flayedwing_transporter_note'] = 'Нажмите на {npc:178633}, чтобы перелететь в Хранилище Тайн или из него.'
L['fleshwing_note'] = 'Поговорите с {npc:180079}, чтобы начать событие сбора.'
L['forgotten_feather_note'] = 'Находится на небольшом висящем в воздухе острове, доступ к которому можно получить, спрыгнув с Приюта Хранителя.'
L['konthrogz_note'] = 'Появляется в событии портала Пожирателя. Событие может появиться во многих местах по всей Кортии.'
L['sl_limited_rare'] = '|cFFFF8C00Этот редкий монстр недоступен в некоторые дни.|r'
L['krelva_note'] = 'Переходит на другую платформу на 80%, затем перемещается на материк на 60%. |cFFFF8C00Вы должны атаковать редкого монстра после 60%, чтобы получить с него добычу.!|r'
L['kroke_note'] = 'Убивайте {npc:179029} поблизости, пока он не появится. Не может появиться в дни, когда отсутствует {npc:179029}.'
L['maelie_wanderer'] = '{npc:179912} появится в случайном месте в течение дня. После того, как вы использовали {spell:355862} в течении 6 дней, вернитесь к {npc:179930}, чтобы забрать средство передвижения.'
L['malbog_note'] = 'Поговорите с {npc:179729}, чтобы получить {spell:355078}, и следуйте по следам, пока не найдете останки плоти.'
L['offering_box_note'] = 'Требуется небольшой ключ для подношения, который можно найти на верхней части западной стены ближайших руин.'
L['pop_quiz_note'] = 'Событие Контрольная будет появляться на карте случайным образом. Нажмите на Брошенный Посох завесы и ответьте на вопросы {npc:180162}'
L['razorwing_note'] = 'Сдайте 10 {item:187054}, полученных с пожирателей в этом районе.'
L['reliwik_note'] = 'Нажмите на нетронутое порчей яйцо бритвокрыла, чтобы призвать монстра.'
L['spectral_bound_chest'] = 'Нажмите на 3 ближайших Призрачных ключа, чтобы открыть сундук.'
L['spectral_bound_note'] = 'Сундук с призрачным замком'
L['stonecrusher_note'] = 'Поговорите с {npc:179974}, чтобы начать событие.'
L['towering_exterminator_note'] = 'Появляется из портала Верных Утробе. Событие может появиться во многих местах по всей Кортии.'
L['worldcracker_note'] = 'Поговорите с {npc:180028}, чтобы вызвать событие сопровождения.'

L['archivist_key_note'] = 'Купите %s у {npc:178257}, чтобы разблокировать.'
L['korthian_shrine_note'] = 'Нажмите на святыню, чтобы получить {spell:352367}, чтобы увидеть скрытый путь к алтарю.'
L['num_research'] = '%d Исследования'
L['plus_research'] = '+Исследования'
L['options_icons_relic'] = '{achievement:15066}'
L['options_icons_relic_desc'] = 'Покажите расположение всех 20 реликвий для {achievement:15066}.'

L['rift_portal_note'] = [[
Войдите |cFFFFFF00в разлом|r, альтернативную фазу Кортии и Утробы с дополнительными редкими монстрами, реликвиями и тайниками.

Требуется {item:186731}, который можно приобрести у {npc:178257}, как только вы достигнете 4 уровня с {faction:2472}. Ключи также имеют низкую вероятность выпадения из редких монстров и тайников в зоне.

|cFFFF8C00Не все порталы разломов активны в любой момент времени.|r
]]
L['rift_rare_only_note'] = 'Этого редкого монстра можно увидеть и убить только внутри фазы |cFFFFFF00разлома|r.'
L['rift_rare_exit_note'] = [[
Этот редкий монстр выходит из |cFFFFFF00разлома|r после взаимодействия внутри разлома.

Три редких монстра из разлома обычно появляются в установленном порядке с интервалом примерно в 20 минут:

  1. {npc:179913}
  2. {npc:179608}
  3. {npc:179911}
]]
L['options_icons_rift_portal'] = '{npc:179595}'
L['options_icons_rift_portal_desc'] = 'Отобразить местоположение {npc:179595}, используемых для входа |cFFFFFF00в разломы|r.'

L['riftbound_cache'] = 'Тайник Разлома'
L['riftbound_cache_note'] = 'Есть 4 уникальных тайника, каждый из которых может появиться в определенных местах внутри разлома.'
L['options_icons_riftbound_cache'] = 'Тайник Разлома'
L['options_icons_riftbound_cache_desc'] = 'Отобразите расположение тайников внутри |cFFFFFF00разломов|r.'

L['invasive_mawshroom'] = 'Инвазивный гриб Утробы'
L['invasive_mawshroom_note'] = 'Есть 5 уникальных грибов, каждый из которых может появиться в определенных местах.'
L['mawsworn_cache'] = 'Тайник Верных Утробе'
L['mawsworn_cache_note'] = 'Есть 3 уникальных тайника, каждый из которых может появляться в определенных местах.'
L['pile_of_bones'] = 'Груда костей'
L['relic_cache'] = 'Тайник с реликвиями'
L['shardhide_stash'] = 'Тайник сланцевика'
L['korthia_shared_chest_note'] = 'Можно добыть 5 реликвии за раз. Прогресс сбрасывается каждые 30 минут, что делает их практически неограниченными.'
L['unusual_nest'] = 'Гнездо с необычными материалами'
L['unusual_nest_note'] = 'Все 5 гнезд можно разграбить каждый день.'

L['options_icons_invasive_mawshroom_desc'] = 'Показать местонахождение инвазивных грибов.'
L['options_icons_invasive_mawshroom'] = 'Инвазивные грибы (ежедневно)'
L['options_icons_korthia_dailies_desc'] = 'Отображение местоположения неотмеченных тайников с реликвиями.'
L['options_icons_korthia_dailies'] = 'Тайник с реликвиями'
L['options_icons_mawsworn_cache_desc'] = 'Показать местонахождение тайников верных утробе.'
L['options_icons_mawsworn_cache'] = 'Тайники Верных Утробе (ежедневно)'
L['options_icons_nest_materials_desc'] = 'Показать местонахождение Гнезд с необычными материалами.'
L['options_icons_nest_materials'] = 'Гнезда с необычными материалами (ежедневно)'

-------------------------------------------------------------------------------
--------------------------------- MALDRAXXUS ----------------------------------
-------------------------------------------------------------------------------

L['chelicerae_note'] = 'Уничтожьте {npc:159885}, чтобы активировать редкого монстра.'
L['deepscar_note'] = 'Может появляться у нескольких входов в Театр боли.'
L['forgotten_mementos'] = 'Потяните цепь опускной решетки хранилища в зале к западу от сундука, чтобы открыть ворота.'
L['gieger_note'] = 'Потяните за {npc:162815}, чтобы активировать редкого монстра.'
L['gristlebeak_note'] = 'Разбейте все {npc:162761} поблизости, чтобы атаковать редких монстров.'
L['leeda_note'] = 'Убейте двух {npc:162220}, пока не появится редкий монстр.'
L['nirvaska_note'] = 'Появляется только когда активно локальное задание {wq:Смертоносное напоминание}.'
L['ravenomous_note'] = 'Давите нейтральных {npc:159901} в этой области, пока не появится редкий монстр.'
L['sabriel_note'] = 'Может появиться как один из чемпионов в Театре боли.'
L['schmitd_note'] = 'Используйте поблизости {spell:313451}, чтобы сломать его щит.'
L['tahonta_note'] = 'Средство передвижение выпадает только в том случае, если с вами {npc:159239}!'
L['taskmaster_xox_note'] = 'Делит появление с {npc:160204}, {npc:160230} и {npc:160226}.'
L['theater_of_pain_note'] = 'При первом убийстве босса каждый день есть шанс получить средство передвижение.'
L['zargox_the_reborn_note'] = [[
Используйте {item:175841} на {npc:157124}. Чтобы получить сферу, вы должны выполнить задание {quest:57245} у {npc:157076} и снова поговорить с ним.

Если {npc:157124} не работает, реанимируйте {npc:157132} в области, пока он не появится.
]]
L['mixed_pool_note'] = [[
Соберите ингредиенты с окружающих монстров и бросьте их в бассейн. После добавления 30 ингредиентов появится один из семи редких монстров в зависимости от используемой комбинации.

• Соберите {spell:306713} с {npc:167923} и {npc:167948} на севере.

• Соберите {spell:306719} с {npc:165015} и {npc:171142} на юге.

• Соберите {spell:306722} с {npc:165027} и {npc:166438} на юге.

Убейте каждого редкого монстра один раз, чтобы получить игрушку {item:183903}.
]]

L['blackhound_cache'] = 'Тайник заставы Черного пса'
L['blackhound_cache_note'] = 'Призовите {npc:157843} на фабрике поганищ, затем сопроводите его на заставу Черного Пса.'
L['bladesworn_supply_cache'] = 'Тайник с припасами присягнувших клинку'
L['cache_of_eyes'] = 'Тайник дома Глаз'
L['cache_of_eyes_note'] = 'Появляется в нескольких местах в пределах Слепой цитадели.'

L['glutharns_note'] = 'В пещере за водопадом слизи. Убейте {npc:172485} и обоих {npc:172479}, чтобы открыть сундук.'
L['kyrian_keepsake_note'] = 'Осмотрите {npc:169664}, чтобы добыть сокровище.'
L['misplaced_supplies'] = 'На вершине гигантского гриба. Бегите на холм и прыгайте на меньший коричневый гриб, затем бегите на большой холм и прыгайте на гигантский гриб.'
L['necro_tome_note'] = 'Чтобы получить доступ к башне, вы должны начать небольшую цепочку заданий у {npc:166657}. В книжном шкафу на верхнем этаже.'
L['plaguefallen_chest_note'] = [[
Встаньте в зеленую слизь (требуется исцеление!), Чтобы получить 10 стаков {spell:330069} и превратиться в {spell:330092}.

После трансформации посетите пещеру под платформой {npc:158406} (вход с восточной стороны) и нажмите на трубу, чтобы переместиться к сундуку.
]]
L['ritualists_cache_note'] = 'Возьмите {item:181558} из груды костей (несколько мест возрождения). Используйте их, чтобы собрать книгу связывающих ритуалов на стенде позади {npc:174386} и получить бафф {spell:337041}.'
L['runespeakers_trove_note'] = 'Найдите {npc:170563} на востоке и убейте его, чтобы получить {item:181777}.'
L['stolen_jar_note'] = 'В пещере под гигантским грибом.'
L['strange_growth_note'] = 'Потяните за {npc:165037}, чтобы найти сокровище.'
L['vat_of_slime_note'] = 'Нажмите на бутылку на столе и затем нажмите на чан со слизью.'

L['giant_cache_of_epic_treasure'] = 'Огромный тайник с эпическим сокровищем'
L['spinebug_note'] = [[
О, смотри! {spell:343124}! Подойди к нему отважный авантюрист, это уж точно не уловка. Подождите, это музыка из Каражана? Что здесь делает {npc:174663} ...?

{spell:343163}!
]]

L['oonar_sorrowbane_note'] = [[
В Театре Боли вы найдете {item:180273} застрявшим в земле и прикрепленным к нему {item:181164}. Вытащить их:

• Купите {item:182163} у {npc:171808} в Ревендрете.
• Купите {item:180771} у {npc:166640} в Мальдраксусе.
• Купите {item:181163} у {npc:169964} в Мальдраксусе.
• Получите 2 стака {spell:306272} во время локального задания {wq:Небольшие трудности} на западе.
• Съешьте 4 раза {spell:327367} в Пустошах Глутарна.
• Быстро используйте {item:181163}, выпейте оба зелья и вытащите руку и меч.

Если хотите вытащить только руку, нужно 4 {spell:327367}.
]]

L['pet_cat'] = 'Погладить проклятого кота!'
L['hairball'] = ns.color.Red('Появляется только в Гниющем святилище внутри подземелья Чумные каскады!')
L['lime'] = 'Находится на вершине большой кости.'
L['moldstopheles'] = 'Бегите по задней части стебля и подпрыгивайте на грибных платформах. Для последней платформы используйте маунта и прыгайте в угол, где он встречается со стеблем.'
L['pus_in_boots'] = 'Расположен под мостом.'

L['options_icons_slime_cat'] = '{achievement:14634}'
L['options_icons_slime_cat_desc'] = 'Отображение местоположения котят для достижения {achievement:14634}.'

L['dundley_note'] = 'Я буду сражаться за свой путь к победе и получу уважение, которое наконец заслуживаю. Единственный недостаток - все, что у меня есть, теперь липкое. Все.'
L['maximillian_note'] = 'Я десятки лет ждал достойного соперника. К победителю идут трофеи!'
L['rotgut_note'] = 'Гнилобрюх. Остатки. Дополнительные части. Ты сражаешься.'

L['ashen_ink_label'] = '{item:183690}'
L['ashen_ink_note'] = 'Случайная добыча из {npc:157125}'

L['jagged_bonesaw_label'] = '{item:183692}'
L['jagged_bonesaw_note'] = 'Случайная добыча из {npc:159105}'

L['discarded_grimoire_label'] = '{item:183394}'
L['discarded_grimoire_note'] = 'Выполните {quest:62297} у {npc:174020}'

L['sorcerers_blade_label'] = '{item:183397}'
L['sorcerers_blade_note'] = 'Выполните {quest:62317} у |cFFFFFF00Записки чародея|r внутри |cFFFFFF00Хранилище душ|r. Вниз по лестнице и налево на книжном шкафу.'

L['mucosal_pigment_label'] = '{item:183691}'
L['mucosal_pigment_note'] = 'Выпадает из любой слизи вокруг |cFFFFFF00Дома чумы|r или в окрестностях {npc:162727}'

L['amethystine_dye_label'] = '{item:183401}'
L['amethystine_dye_note'] = 'Выполните {quest:62320} у {npc:174120}'

L['ritualists_mantle_label'] = '{item:183399}'
L['ritualists_mantle_note'] = 'Выполните {quest:62308} у {npc:172813}. Требуется 3 игрока, чтобы вызвать {npc:174127}.'

L['options_icons_crypt_couture'] = '{achievement:14763}'
L['options_icons_crypt_couture_desc'] = 'Некроаколит маскирует вас в этой местности для {achievement:14763}'

L['soulshape_saurid_note'] = 'В небольшой пещере. Возьмите в цель {npc:182105} и используйте эмоцию /поклон'

-------------------------------------------------------------------------------
--------------------------------- REVENDRETH ----------------------------------
-------------------------------------------------------------------------------

L['amalgamation_of_filth_note'] = 'При локальном задании {wq:Грязная работенка: обрушение} появляются коробки с мусором и используйте {spell:324115} в воду.'
L['amalgamation_of_light_note'] = 'Переместите все три зеркальные ловушки, чтобы выпустить редкого монстра.'
L['amalgamation_of_sin_note'] = 'Во время локального задания {wq:Призыв своих грехов} выберите Катализатор силы, чтобы получить шанс получить {item:180376}, затем используйте этот предмет, чтобы призвать редкого монстра.'
L['bog_beast_note'] = 'Имеет шанс появиться во время локального задания {wq:Грязища} после использования {item:177880} на {npc:166206}.'
L['endlurker_note'] = 'Нажмите на шип анимы возле трупа {npc:165229} и используйте {spell:321826} на вершине мерцающего разлома.'
L['executioner_aatron_note'] = 'Убейте трех ближайших {npc:166715}, чтобы удалить {spell:324872}.'
L['executioner_adrastia_note'] = 'Освобождайте {npc:161299} в окрестностях и сопровождайте их, пока они не исчезнут. {npc:161310} в конечном итоге появится, чтобы подавить восстание.'
L['famu_note'] = 'Поговорите с {npc:166483}, чтобы запустить событие.'
L['grand_arcanist_dimitri_note'] = 'Убейте четырех {npc:167467}, чтобы выпустить редкого монстра.'
L['harika_note'] = 'В жуткой лощине на западе возьмите {item:176397}, затем отдайте его {npc:165327} и скажите ему, чтобы он сбил редкого монстра.'
L['innervus_note'] = 'Убейте поблизости {npc:160375}, чтобы получить {item:177223} и открыть склеп.'
L['leeched_soul_note'] = 'Внутри ближайшего склепа. Подойдите к {npc:165151}, чтобы начать событие.'
L['lord_mortegore_note'] = 'Убейте окружающих монстров, чтобы получить {item:174378} и использовать его, чтобы усилить {npc:161870}. Редкий монстр появится после того, как будут усилены все четыре печати.'
L['madalav_note'] = 'Нажмите на Молот Мадалава на ближайшей наковальне, чтобы призвать его.'
L['manifestation_of_wrath_note'] = 'Имеет шанс появиться, когда {npc:169916} восстанавливается во время локального задания {wq:Рой душ}.'
L['scrivener_lenua_note'] = 'Верните {npc:160753} в запрещенную библиотеку.'
L['sinstone_hoarder_note'] = 'Попытайтесь открыть {npc:162503}, и редкий монстр появится.'
L['sire_ladinas_note'] = 'Подберите Частицу Света поблизости и используйте {spell:313065} на {npc:157733}.'
L['soulstalker_doina_note'] = 'Следуй вниз и пройди через зеркало, когда она убегает.'
L['tomb_burster_note'] = 'Может появиться, если {npc:155777} пойман в сети. Убивайте ближайших {npc:155769} и появившихся {npc:155795}, пока не появится редкий монстр.'
L['worldedge_gorger_note'] = [[
Получите {item:173939} из обитателей гибельного леса. Используйте его, чтобы зажечь жаровни края мира и вызвать редкого монстра.

После получения {item:180583}, используйте данный предмет и у вас начинается 7-дневная цепочка заданий для получения средства передвижения {spell:333027}.
]]

L['dredglaive_note'] = 'Под мостом в трупе {npc:173671}.'
L['forbidden_chamber_note'] = 'Найдите выброшенную канистру анимы перед запертой дверью и выучите {spell:340701}. Используйте его, чтобы осушить пять ближайших {npc:173838}, затем используйте {spell:340866} перед {npc:173786}.'
L['gilded_plum_chest_note'] = 'Убейте {npc:166680}, бродящего по дороге.'
L['lost_quill_note'] = 'Возьмите {item:182475} из бутылки на столе в запретной библиотеке.'
L['rapier_fearless_note'] = 'Используйте рапиру на земле, затем победите {npc:173603}.'
L['remlates_cache_note'] = 'На внешней стене темного приюта за склепом.'
L['smuggled_cache_note'] = ns.color.Red('ВАЖНО: ') .. 'Убедитесь, что вы выполнили побочное задание {quest:60480}, иначе вы пропустите 40 {currency:1820} из сокровищницы и задания.'
L['taskmaster_trove_note'] = 'Прочтите Ритуалы прихода и ухода, затем осторожно пройдите к сундуку.'
L['the_count_note'] = 'Наберите 99 {currency:1820} в Гиблотопе и принесите их к {npc:173488}.'

L['forgotten_anglers_rod'] = 'Забытая удочка'

L['loyal_gorger_note'] = 'Посетите гиблотопь и завершите ежедневное задание, предлагаемое вашим {npc:173498} 7 раз, чтобы получить его в качестве средства передвижения.'

L['sinrunner_note'] = 'Приносите еду и припасы {npc:173468} в течении шести дней, чтобы получить ее поводья. Она появляется только на несколько минут.'
L['sinrunner_note_day1'] = 'Предложите Бланчи 8 {item:182581} с фермы в Западном крае.'
L['sinrunner_note_day2'] = 'Получите {item:182585} у {npc:173570} в Темном Приюте и используйте ее, чтобы очистить Бланчи.'
L['sinrunner_note_day3'] = 'Оденьте на Бланчи 4 {item:182595}, найденных на дорогах вокруг Темного Приюта.'
L['sinrunner_note_day4'] = 'Возьмите {item:182620} рядом с {npc:173570} и наполните его водой из Бастиона или Арденвельда. Верните {item:182599} Бланчи.'
L['sinrunner_note_day5'] = 'Оснастите Бланчи {item:182597}, которое обменивается у {npc:171808} возле ночного рынка на различное мясо из текущей локации.'
L['sinrunner_note_day6'] = 'Купите 3 {item:179271}, у {npc:167815} для Бланчи.'

L['options_icons_carriages'] = '{achievement:14771}'
L['options_icons_carriages_desc'] = 'Отображение местоположения и пути проезжаемых экипажей.'
L['options_icons_dredbats'] = '{achievement:14769}'
L['options_icons_dredbats_desc'] = 'Показать местоположения и пути {npc:161015}.'
L['options_icons_sinrunners'] = '{achievement:14770}'
L['options_icons_sinrunners_desc'] = 'Показать местоположения и пути {npc:174032}.'

L['addius_note'] = 'Слабоумным существам не стоит тратить мое время, но если вы будете настаивать, я покажу вам настоящую боль.'
L['eyegor_note'] = 'Глазгор готов к бою!'
L['sylla_note'] = 'Никогда нельзя ожидать выдающейся битвы здесь, в этом ужасном месте, но, увы, мы здесь. Не трать мое время.'

L['avowed_ritualist_note'] = 'Принесите сюда {npc:160149}, чтобы освободить их.'
L['fugitive_soul_note'] = 'Отнесите этот {npc:160149} к ближайшему {npc:166150}, чтобы начать ритуал отпущения грехов.'
L['souls_absolved'] = 'души отпущены'
L['options_icons_fugitives'] = '{achievement:14274}'
L['options_icons_fugitives_desc'] = 'Показать местонахождение беглых душ для достижения {achievement:14274}.'

L['grand_inquisitor_note'] = 'Сдайте 10 {item:180451} у {npc:160248}, чтобы получить шанс на этот камень греха.'
L['high_inquisitor_note'] = 'Сдайте 250 {currency:1816} у {npc:160248}, чтобы получить шанс получить этот камень греха.'
L['inquisitor_note'] = 'Сдайте 100 {currency:1816} у {npc:160248}, чтобы получить шанс получить этот камень греха.'
L['options_icons_inquisitors'] = 'Инквизиторы'
L['options_icons_inquisitors_desc'] = 'Показать местоположения инквизиторов для достижения {achievement:14276}.'

L['bell_of_shame_note'] = 'Каждые 30 минут рядом с {npc:176006}.\n\nЧините {npc:176056} за 30 {currency:1820} а затем позвоните в колокол, чтобы получить бафф от активного призрака.\n\nОдин из призраков, {npc:176043}, дает {spell:346708} что обеспечивает повышенный дроп {item:172957} на территории |cFFFFFD00Чертогов Покаяния|r.'
L['atonement_crypt_label'] = 'Склеп Покаяния'
L['atonement_crypt_note'] = 'Откройте |cFFFFFD00Склеп Покаяния|r используя {item:172957}.'
L['atonement_crypts_opened'] = 'Склепы Покаяния открыты'
L['atonement_crypt_key_label'] = '{item:172957}'
L['atonement_crypt_key_note'] = '{item:172957} может выпасть из большинства монстров на территории |cFFFFFD00Чертогов Покаяния|r.\n\n{npc:158902}\n{npc:176109}\n{npc:158894}\n{npc:156911}\n{npc:158910}\n{npc:176121}\n{npc:176114}\n{npc:156909}\n{npc:156256}\n{npc:176124}\n{npc:156260}\n{npc:159027}\n{npc:158897}\n{npc:176116}\n{npc:158908}\n{npc:176122}\n\nСамый высокий процент выпадения из {npc:158892}.'
L['options_icons_crypt_kicker'] = '{achievement:14273}'
L['options_icons_crypt_kicker_desc'] = 'Отображает места для достижения {achievement:14273}.'

L['broken_mirror'] = 'Сломанные зеркала'
L['broken_mirror_note'] = 'Группа из трех разбитых зеркал будет активна каждый день. Используйте {item:181363}, чтобы починить каждое зеркало и открыть |cFFFFFD00Позабытый сундук|r внутри.'
L['broken_mirror_crypt'] = 'Внутри склепа.'
L['broken_mirror_elite'] = 'В маленькой комнате с элитными монстрами.'
L['broken_mirror_group'] = 'Группа'
L['broken_mirror_house'] = 'Внутри дома.'
L['broken_mirror_61818'] = 'В маленькой комнате с {npc:173699}.'
L['broken_mirror_61819'] = 'В маленькой комнате на первом этаже.'
L['broken_mirror_61827'] = 'В маленькой комнате.'
L['options_icons_broken_mirror'] = 'Сломанные зеркала'
L['options_icons_broken_mirror_desc'] = 'Показать расположение сломанных зеркал.'

L['soulshape_chicken_note'] = [[
1. Соберите {item:187811} в |cffffff00Темном Приюте|r
2. Взять в цель {npc:181660} и используйте эмоцию /цып
3. |cFFFF8C00Потерянная душа жадно кудахчет вам.|r
4. Взять в цель {npc:181660} и использовать {item:187811}
]]
L['spectral_feed_label'] = '{item:187811}'
L['spectral_feed_note'] = [[
{item:187811} используется для кормления {npc:181660} в |cffffff00Ревендрете|r для {item:187813}

{item:187811} используется для кормления {npc:182093} в |cFFFFFF00Арденвельде|r для |cFF00FF00[Сытой кошачьей души]|r

|cFFFF8C00{item:187811} имеет 10-минутный таймер и 60-минутное возрождение|r
]]

-------------------------------------------------------------------------------
----------------------------------- THE MAW -----------------------------------
-------------------------------------------------------------------------------

L['return_to_the_maw'] = 'Возвращение в Утробу'
L['maw_intro_note'] = 'Начните вводную цепочку заданий у {npc:162804}, чтобы разблокировать редких монстров и события в Утробе.'

L['apholeias_note'] = 'Вместе с 3 другими игроками встаньте на углы платформы и примените {spell:331783}, чтобы призвать редкого монстра.'
L['dekaris_note'] = 'На вершине большой выступающей скалы.'
L['deomen_note'] = 'Чтобы открыть его камеру, войдите в комнату на юге и активируйте оба замка с обеих сторон комнаты.'
L['drifting_sorrow_note'] = 'Убейте {npc:175246} рядом с парящей сферой, чтобы активировать босса.'
L['ekphoras_note'] = 'Вместе с 3 другими игроками встаньте на углы платформы и примените {spell:330650}, чтобы призвать редкого монстра.'
L['etherwyrm_note'] = 'Требуется активная атака Ночного Народца. Убейте {npc:179030} в разломе, чтобы получить {item:186190}. Используйте ключ на клетке для эфирных змеев в Заброшенном Гроте (где находится {npc:175821}, когда штурм неактивен).'
L['fallen_charger_note'] = 'После крика который слышно по всей локации он пойдет по одному из двух путей, пока не достигнет Кортии, где исчезнет.'
L['ikras_note'] = 'Летает вокруг Цитадели Вечных Мук. Это хорошее место, чтобы его атаковать.'
L['lilabom_note'] = [[
Соберите все 5 частей, чтобы получить питомца. Некоторые части могут появляться в нескольких местах.

• {item:186183}
• {item:186184}
• {item:186185}
• {item:186186}
• {item:186187}
]]
L['orophea_note'] = 'Поднимите {spell:337143} на юго-востоке и предложите {npc:172577}, чтобы активировать.'
L['sanngror_note'] = 'Если его нельзя атаковать, подождите, пока он не перестанет экспериментировать с душами.'
L['sly_note'] = 'Поговорите с {npc:179068}, чтобы получить бафф {spell:353322} и найти {npc:179096} на 3 различных Кирийских нападениях.'
L['talaporas_note'] = 'Вместе с 3 другими игроками встаньте на углы платформы и примените {spell:331800}, чтобы призвать редкого монстра.'
L['valis_note'] = 'Нажмите на три {npc:174810} в правильном порядке, чтобы вызвать редкого монстра. Порядок меняется каждый раз, и нажатие не на ту руну наносит урон и снимает с вас дебаффы {spell:343636}.'
L['yero_note'] = 'Подойдите к {npc:172862} и затем следуйте за ним в ближайшую пещеру, где он станет враждебным.'

L['exos_note'] = [[
Убейте трех вестников скорби, боли и утраты, чтобы собрать их гравировки.

• {item:182328}
• {item:182326}
• {item:182327}

Объедините все три гравировки, чтобы создать {item:182329}, который можно использовать для прохода к Алтарю Господства. Используйте {npc:173892}, чтобы добраться до верхнего уровня.
]]

L['animaflow_teleporter_note'] = 'Активируйте, чтобы отправиться прямо в другие места в Утробе.'
L['chaotic_riftstone_note'] = 'Активируйте, чтобы быстро {spell:344157} перейти через Утробу.'
L['venari_note'] = [[
Используйте {currency:1767} для покупки улучшений в Утробе и Торгасте.

|cFFFF8C00Улучшения для Торгаста распространяются на всю учетную запись. Однако они будут отображаться как незавершенные на альтернативных персонажах!|r
]]
L['venari_upgrade'] = '{npc:162804} Улучшить'
L['torghast'] = 'Торгаст'
L['Ambivalent'] = 'с безразличием'
L['Appreciative'] = 'с признанием'
L['Apprehensive'] = 'с опаской'
L['Cordial'] = 'с сердечностью'
L['Tentative'] = 'с настороженностью'

L['stygian_cache'] = 'Стигийский тайник'
L['stygian_cache_note'] = 'Только один человек может открыть тайник каждый раз, когда он появляется!'

L['box_of_torments_note'] = 'Откройте {npc:173837} под Тремакулюмом.'
L['tormentors_notes_note'] = 'Обыщите труп {npc:173811}.'
L['words_of_warden_note'] = 'Осмотрите |cffffff00Обрывок бумаги|r на камне за горшками.'

-- Locations given relative to a map area name
L['nexus_area_calcis_branch'] = 'На кристальной ветке в Кальцисе (используйте точку захвата)'
L['nexus_area_calcis_crystals'] = 'За бирюзовыми кристаллами в Кальцисе'
L['nexus_area_cradle_bridge'] = 'Под мостом в Колыбели Разрушения'
L['nexus_area_domination_bridge'] = 'На мосту к югу от Алтаря Господства'
L['nexus_area_domination_edge'] = 'На краю Алтаря Господства'
L['nexus_area_domination_room'] = 'В маленькой комнате на Алтаре Господства'
L['nexus_area_domination_stairs'] = 'На Алтаре Господства рядом с {npc:173904}'
L['nexus_area_gorgoa_bank'] = 'На берегу Горгоа (Реки Душ)'
L['nexus_area_gorgoa_middle'] = 'Прямо посреди реки!'
L['nexus_area_gorgoa_mouth'] = 'В устье Горгоа (Реки Душ)'
L['nexus_area_perdition_wall'] = 'Вдоль внешней стены Цитадель Вечных Мук'
L['nexus_area_torment_rock'] = 'На скале в Обители мучений'
L['nexus_area_zone_edge'] = 'По краю зоны'
L['nexus_area_zovaal_edge'] = 'По краю Котла Зоваала'
L['nexus_area_zovaal_wall'] = 'Вдоль стены под Котлом Зоваала'
-- Locations given relative to a named cave/cavern
L['nexus_cave_anguish_lower'] = 'В Яме Страданий (нижний уровень)'
L['nexus_cave_anguish_outside'] = 'За пределами Ямы Страданий'
L['nexus_cave_anguish_upper'] = 'В Яме Страданий (верхний уровень)'
L['nexus_cave_desmotaeron'] = 'В маленькой пещере за пределами Дезмотерона'
L['nexus_cave_echoing_outside'] = 'За пределами Гулких пещер'
L['nexus_cave_forlorn'] = 'Внутри пещеры Тоскливый Приют.'
L['nexus_cave_howl_outside'] = 'На земле за пределами пещеры Вой Смерти.'
L['nexus_cave_howl'] = 'Внутри пещеры Вой Смерти'
L['nexus_cave_roar'] = 'Внутри пещеры Рев Смерти'
L['nexus_cave_roar_outside'] = nil
L['nexus_cave_ledge'] = 'В маленькой пещере под уступом'
L['nexus_cave_prodigum'] = 'В маленькой пещере в Продигум'
L['nexus_cave_soulstained'] = 'В маленькой пещере в Полях Оскверненных Душ'
L['nexus_cave_torturer'] = 'Внутри Дома Истязателя'
-- Locations given relative to a named NPC
L['nexus_npc_akros'] = 'Наверху лестницы рядом с {npc:170787}'
L['nexus_npc_dekaris'] = 'На вершине пика, где находится {npc:157964}'
L['nexus_npc_dolos'] = 'На земле позади {npc:170711}'
L['nexus_npc_ekphoras'] = 'Рядом с платформой {npc:169827}'
L['nexus_npc_eternas'] = nil
L['nexus_npc_incinerator'] = 'На небольшом выступе под {npc:156203}'
L['nexus_npc_orophea'] = 'На земле рядом с {npc:172577}'
L['nexus_npc_orrholyn'] = 'Найдено ниже платформы {npc:162845}'
L['nexus_npc_portal'] = 'На небольшом камне за {npc:167531}'
L['nexus_npc_talaporas'] = 'У лестницы на платформу {npc:170302}'
L['nexus_npc_thanassos'] = 'На задней стороне платформы {npc:170731}'
L['nexus_npc_willbreaker'] = 'В углу за {npc:168233}'
-- Locations given relative to the main path/road nearby
L['nexus_road_below'] = 'На земле под главной дорогой'
L['nexus_road_cave'] = 'В маленькой пещере под дорогой'
L['nexus_road_mawrats'] = 'В стае крыс утробы у дороги'
L['nexus_road_next'] = 'Рядом с главной дорогой'
L['nexus_room_ramparts'] = 'В маленькой комнате под стеной'
-- Random locations described as best as possible
L['nexus_misc_crystal_ledge'] = 'На каменном выступе рядом с какими-то бирюзовыми кристаллами'
L['nexus_misc_floating_cage'] = 'На висящей клетке (используйте точки захвата)'
L['nexus_misc_below_ramparts'] = 'Под нижней частью стены'
L['nexus_misc_grapple_ramparts'] = 'На вершине стены (используйте точку захвата)'
L['nexus_misc_grapple_rock'] = 'За точкой захвата на скале'
L['nexus_misc_ledge_below'] = 'На земле под выступом'
L['nexus_misc_three_chains'] = 'На земле за тремя цепями'

L['stolen_anima_vessel'] = 'Сосуд с украденной анимой'
L['hidden_anima_cache'] = 'Тайник с украденной анимой'
L['options_icons_anima_vessel'] = 'Сосуд с украденной анимой'
L['options_icons_anima_vessel_desc'] = 'Отображение местоположения сосудов анимы во время атак и внутри |cFFFFFF00разломов|r.'

L['rift_hidden_cache'] = 'Тайник разлома'
L['options_icons_rift_hidden_cache'] = 'Тайник разлома'
L['options_icons_rift_hidden_cache_desc'] = 'Показать расположение тайников внутри |cFFFFFF00разломов|r.'

L['options_icons_bonus_boss'] = '{achievement:14660}'
L['options_icons_bonus_boss_desc'] = 'Отображение местоположения бонусных элитных монстров.'
L['options_icons_riftstone'] = '{npc:174962}'
L['options_icons_riftstone_desc'] = 'Отображение местоположения хаотичных каменей разлома.'
L['options_icons_grapples'] = '{npc:176308}'
L['options_icons_grapples_desc'] = 'Отображение местоположения крюков для улучшения {item:184653}.'
L['options_icons_stygia_nexus'] = 'Скопление стигии'
L['options_icons_stygia_nexus_desc'] = 'Показать местоположения скоплений, необходимые для {item:184168}.'
L['options_icons_stygian_caches'] = 'Стигийский тайник'
L['options_icons_stygian_caches_desc'] = 'Показать места тайников, содержащих Стигию.'

L['cov_assault_only'] = 'Доступно только во время атаки ковенантов %s.'

L['helgarde_supply'] = 'Хельгардский тайник с припасами'
L['helgarde_supply_note'] = 'Появляется по всей территории Дезмотерон. Ваш {npc:180598} поможет вам их найти.'
L['options_icons_helgarde_cache'] = 'Хельгардский тайник с припасами'
L['options_icons_helgarde_cache_desc'] = 'Показать возможные местоположения Хельгардских тайников с припасами в районе Дезмотерона.'

L['mawsworn_cache_ramparts_note'] = 'Этот тайник находится на вершине крепостных стен. Используйте точки захвата или {npc:177093}, чтобы добраться до них.'
L['mawsworn_cache_tower_note'] = 'Этот тайник находится наверху башни, и для его доступа требуется {npc:177093}. Используйте способность {spell:349853}, чтобы взобраться наверх.'
L['mawsworn_cache_quest_note'] = '{item:186573} выпадет только после того, как вы выполните задание {quest:63545}!'

L['nilg_silver_ring_note'] = 'Соберите 4 {item:186727} в Дезмотероне и используйте их, чтобы открыть Запечатанный Сундук Господства.'
L['nilg_silver_ring_note1'] = 'Убейте {npc:177444} и откройте {item:186970}.'
L['nilg_silver_ring_note2'] = 'Отыщите кольцо с ключами боронителя душ, которое находится на стене в подземной комнате рядом с {npc:178311}.'
L['nilg_silver_ring_note3'] = 'Откройте Хельгардский тайник с припасами в районе Дезмотерона. Ваш {npc:180598} поможет вам их найти.'
L['nilg_silver_ring_note4'] = 'Убивайте {npc:177134} в районе Дезмотерона (низкий шанс выпадения).'
L['nilg_stone_ring_note'] = 'Соберите четыре {item:186600} во время нападения Некролордов и объедините их в любом {npc:171492} в Котле Зоваала.'
L['nilg_stone_ring_note1'] = 'Можно найти в некоторых тайниках верных Утробе (желтый значок) на вершине крепостных стен в Цитадели Вечных Мук.'
L['nilg_stone_ring_note2'] = 'Завершите задание {quest:63545}, затем открывайте тайники верных Утробе, пока не найдете предмет {item:186573}. Этим заданием можно делиться! В сутки можно открыть 6 сундуков.'
L['nilg_stone_ring_note3'] = 'Добывается из {npc:179601} в центре Цитадели Вечных Мук.'
L['nilg_stone_ring_note4'] = 'Найден на земле в Цитадели Вечных Мук рядом с {npc:170634}. Ваш {npc:180598} может помочь вам найти его.'
L['nilg_gold_band_note'] = 'Используйте точку захвата и следуйте по тропинке на вершину горы.'
L['nilganihmaht_note'] = 'Вы должны собрать 5 колец и принести их {npc:179572} в разлом.'
L['calcis'] = 'Кальцис'
L['desmotaeron'] = 'Дезмотерон'

L['zovault_note'] = 'Отнесите {npc:179883} к {npc:179904} один раз в день, чтобы получить гарантированный коллапсирующий камень разлома.'
L['options_icons_zovault'] = '{npc:179883}'
L['options_icons_zovault_desc'] = 'Показать возможные местоположения для {npc:179883}.'

L['tormentors'] = 'Мучители из Торгаста'
L['tormentors_note'] = [[
Событие мучителей появляется каждые 2 часа. Боссы всегда будут появляться в порядке, указанном ниже.

{item:185972} можно получить только раз в неделю, он содержит 50 {currency:1906}.
]]

L['options_icons_mawsworn_blackguard'] = '{achievement:14742}'
L['options_icons_mawsworn_blackguard_desc'] = 'Mawsworn Blackguard locations for {achievement:14742}'

L['mawsworn_blackguard'] = 'Верный Утробе черный страж'
L['mawsworn_blackguard_note'] = 'Easily target a stealthed {npc:183173}:'

L['options_icons_covenant_assaults'] = 'Covenant Assaults'
L['options_icons_covenant_assaults_desc'] = 'Rewards for covenant assaults'

L['assault_sublabel_US'] = 'Нападения меняются во вторник в 8:00 AM PDT и в пятницу в 20:00 PM PDT.'
L['assault_sublabel_EU'] = 'Нападения меняются в среду в 9:00 по Московскому времени и в субботу в 21:00 по Московскому времени.'
L['assault_sublabel_CN'] = 'Нападения меняются в четверг в 7:00 CST и в воскресенье в 19:00 CST.'
L['assault_sublabel_AS'] = 'Нападения меняются в четверг в 8:00 AM KST и в воскресенье в 20:00 PM KST.'

L['necrolord_assault'] = '{quest:63543}'
L['necrolord_assault_note'] = '{item:185992} можно получить один раз за нападение'
L['necrolord_assault_quantity_note'] = 'Открыть Тайник Верных Утробе'
L['venthyr_assault'] = '{quest:63822}'
L['venthyr_assault_note'] = '{item:185990} можно получить один раз за нападение'
L['venthyr_assault_quantity_note'] = 'Используемые предметы'
L['night_fae_assault'] = '{quest:63823}'
L['night_fae_assault_note'] = '{item:185991} можно получить один раз за нападение'
L['night_fae_assault_quantity_note'] = 'Открыть Тайник разлома'
L['kyrian_assault'] = '{quest:63824}'
L['kyrian_assault_note'] = '{item:185993} можно получить один раз за нападение'
L['kyrian_assault_quantity_note1'] = 'Патрик найден'
L['kyrian_assault_quantity_note2'] = 'Танцы возле кузниц'

-------------------------------------------------------------------------------
---------------------- TORGHAST, THE TOWER OF THE DAMNED ----------------------
-------------------------------------------------------------------------------

L['torghast_the_tower_of_the_damned'] = 'Торгаст, Башня Проклятых'
L['torghast_reward_sublabel'] = '|cFFFF8C00Награды распределяются между большинством крыльев|r'
L['torghast_boss_note'] = 'Выпадает из разных боссов в |cffffff00Торгаст, Башня Проклятых|r '
L['torghast_vendor_note'] = 'Продается у торговцев {npc:152594} и {npc:170257} за 300 {currency:1728} или 1,000 {currency:1728}'
L['torghast_reward_note'] = 'Награды в |cffffff00Торгаст, Башня Проклятых|r '
L['torghast_soulshape_note'] = 'Этаж 12+'
L['colossal_umbrahide_mawrat_note'] = 'Этаж 13+'

L['skoldus_hall'] = 'Чертоги Скольда'
L['fracture_chambers'] = 'Костоломни'
L['the_soulforges'] = 'Кузни душ'
L['coldheart_interstitia'] = 'Междумирье Бессердечных'
L['mortregar'] = 'Морт\'регар'
L['the_upper_reaches'] = 'Верхний ярус'
L['adamant_vaults'] = 'Адамантовые хранилища'
L['twisting_corridors'] = 'Извилистые коридоры'
L['the_jailers_gauntlet'] = 'Испытание Тюремщика'

L['torghast_layer1'] = 'Завершить уровень 1'
L['torghast_layer2'] = 'Завершить уровень 2'
L['torghast_layer3'] = 'Завершить уровень 3'
L['torghast_layer4'] = 'Завершить уровень 4'
L['torghast_layer6'] = 'Завершить уровень 6'
L['torghast_layer8'] = 'Завершить уровень 8'

L['phantasma_note'] = 'Фантазма'
L['bloating_fodder_note'] = 'Взрыв падали'
L['flawless_master_note'] = 'Завершите |cffffff00[{achievement:15322}]|r'
L['tower_ranger_note'] = 'Завершите |cffffff00[{achievement:15324}]|r'

L['the_jailers_gauntlet_note'] = '|cFFFF8C00Боссы не засчитываются|r |cffffff00{achievement:14498}|r'

L['the_box_of_many_things'] = 'Шкатулка многих тайн'
L['the_box_of_many_things_note'] = 'Разблокируйте дополнительные способности с помощью {currency:1904}'
L['many_many_things_section'] = 'Завершите |cffffff00[{achievement:15079}]|r'

L['the_runecarver'] = '{npc:164937}'
L['clearing_the_fog_suffix'] = 'Воспоминания разблокированы'

-------------------------------------------------------------------------------
-------------------------------- ZERETH MORTIS --------------------------------
-------------------------------------------------------------------------------

local HIDDEN_ALCOVE = [[
Чтобы получить доступ к |cFFFFFD00%s Альков|r:

1. Завершите шестую главу кампании Зерет Мортис.
2. Найдите |cFFFFFD00%s конфигурацию Альков|r %s.
3. В |cFFFFFD00Пиках Отголосков|r, Убейте монстров, у которых есть {spell:362651} или стойте в белых лужах, чтобы получить 60 {npc:183569}.
4. Использовать {npc:184329} внутри Приюта Отягощенных, чтобы получить доступ к Внутреннему локусу. (Требует 30 {npc:183569})
5. Использовать {npc:184485} чтобы получить доступ к %s Альков. (Требует 30 {npc:183569})
]]

L['camber_alcove_note'] = string.format(HIDDEN_ALCOVE, 'Выпуклый', 'Выпуклый', 'на задней части пирамиды в |cFFFFFD00Финальном Локусе|r', 'Выпуклый')
L['dormant_alcove_note'] = string.format(HIDDEN_ALCOVE, 'Дремлющий', 'Дремлющий', 'в |cFFFFFD00Пиках Отголосков|r', 'Дремлющий')
L['fulgor_alcove_note'] = string.format(HIDDEN_ALCOVE, 'Ослепительный', 'Ослепительный', 'в |cFFFFFD00Пиках Отголосков|r', 'Ослепительный')
L['rondure_alcove_note'] = string.format(HIDDEN_ALCOVE, 'Округлый', 'Округлый', 'на платформе |cFFFFFD00Сдвига Локуса|r', 'Округлый')
L['repertory_alcove_note'] = string.format(HIDDEN_ALCOVE, 'Постоянный', 'Постоянный', 'внутри пещеры |cFFFFFD00Хранилище земных пород|r', 'Постоянный')

L['corrupted_architect_note'] = 'Задействуйте {npc:183958} и {npc:183961}, чтобы активировать редкого монстра.'
L['dune_dominance_note'] = 'Все три редких монстра для достижения {achievement:15392} появляются в этом месте.'
L['feasting_note'] = 'Иногда будет кружить по области, прежде чем вернуться на место.'
L['furidian_note'] = 'Активировать три |cFFFFFD00усиленных ключа|r в этом районе, затем разблокируйте |cFFFFFD00Подозрительное рассерженное хранилище|r.'
L['garudeon_note'] = 'Соберите {npc:183562} в окрестностях и используйте {spell:362655}, чтобы накормить {npc:183554}. Как только все трое будут накормлены, активируется {npc:180924}.'
L['gluttonous_overgrowth_note'] = 'Уничтожьте всех {npc:184048} поблизости, чтобы активировать редкого монстра.'
L['helmix_note'] = [[
Убивайте {npc:179005} в этом районе, пока не появится эмоция.

|cFFFF8C00Земля под ногами дрожит... Что-то прокапывает путь!|r
]]
L['hirukon_note'] = [[
Чтобы выманить {npc:180978} на поверхность, необходимо создать {item:187923}.

1. Выловите {item:187662} в близлежащих водах.

2. Выловите {item:187915} из |cFFFFFD00косяка едких рыб-капель|r в Резервуаре Кривого Клыка в Зангартопи.

3. Выловите {item:187922} из |cFFFFFD00стаи плавунцов|r возле точки полета Могила Келии в Назжатаре.

4. Найдите (очень хорошо спрятанный) {item:187916} на балконе на втором уровне террасы Нар'шолы в Мерцающих просторах (34.7, 75.0).

5. Найдите {npc:182194} возле пруда к югу от Престола Примаса в Малдраксусе. Попросите ее изготовить {item:187923}.

6. Используйте {item:187923}, чтобы увидеть косяк |cFFFFFD00рой аурелидов|r возле {npc:180978} на 15 минут. Рыбачьте в нем, чтобы призвать {npc:180978} на поверхность.
]] -- Breaking my rule of no coords in the notes for this one
L['orixal_note'] = 'Делит точку возрождения с {npc:185487}, патрулирующим территорию.'
L['protector_first_ones_note'] = 'Требуется два игрока. Принесите соответствующие руны к каждой консоли (руны расположены поблизости), чтобы открыть барьер.'
L['the_engulfer_note'] = 'Защищайте {npc:183505}, пока {npc:183516} не появится из разлома.'
L['zatojin_note'] = 'Атакуйте {npc:183721} и позвольте им наложить 20 стаков {spell:362976} на вас, пока вы не станете {spell:362983}. Вы должны стоять над трупами {npc:183774}, когда вас парализует, чтобы {npc:183764} заметил вас.'

L['architects_reserve_note'] = 'Выполните цепочку заданий {quest:64829} от {npc:180630} в Приюте Пилигрима.'
L['bushel_of_produce_note'] = 'Убивайте {npc:182368} на севере, пока на них действует {spell:360945}, чтобы усилить себя (не обязательно). Убивайте {npc:182368} (им не нужен бафф), пока на вас не будет 5 стаков, затем сломайте дверь к сокровищу.'
L['crushed_crate_note'] = 'Найдите {item:189767}, находится на упавшей колонне над сокровищем. Отдайте его {npc:185151} в обмен на {item:189768}, которым можно разбивать камни.'
L['domination_cache_note'] = '{item:189704} имеет низкий шанс выпасть из {npc:181403} и {npc:182426} в этом районе.'
L['drowned_broker_supplies_note'] = 'С помощью {npc:181059} возьми под контроль ближайших {npc:185282}.'
L['forgotten_protovault_note'] = 'Доступно только во время локального задания {wq:Полный ква-вардак}.'
L['grateful_boon_note'] = 'Трудно добраться без полетов; используйте {spell:300728} или {spell:111771}, чтобы подняться на гору. Погладьте всех 12 существ в этом районе, и {npc:185293} отдаст вам сокровище.'
L['library_vault_note'] = 'Щелкайте по табличкам в Гроте Слов, пока не найдете нужный бафф {spell:362062}, чтобы открыть хранилище.'
L['mistaken_ovoid_note'] = 'Внутри пещеры Водопады Измерений. Вы должны собрать 5x {item:190239} и принести их {npc:185280}.'
L['ripened_protopear_note'] = 'Начните цепочку заданий {quest:64641}, чтобы получить доступ к Литейной Цветения. Соберите 5 {spell:367180} (зеленых облаков) внутри и несите их один за другим к {npc:185416}, чтобы опылить его. Как только вы опылите его 5 раз он отдаст сокровище.'
L['sphere_treasure_note'] = 'Осторожно прыгайте по сферам, пока не доберетесь до вершины.'
L['submerged_chest_note'] = 'Подберите |cFFFFFFD0Опасную сферу могущества|r на юге, затем взаимодействуйте с |cFFFFFD00Забытым насосом|r рядом с сокровищем.'
L['symphonic_vault_note'] = 'Исследуйте {npc:183998}, чтобы услышать последовательность из четырех звуков. Каждый {npc:183950} в комнате воспроизводит один звук. Взаимодействуйте с ними в соответствующем порядке, чтобы разблокировать сокровище.'
L['syntactic_vault_note'] = 'Чтобы разблокировать сокровище, вы должны получить 6 стаков {spell:367499}, взаимодействуя с |cFFFFFD00Руническими слогами|r в районе Гробницы Предвечных.'
L['template_archive_note'] = 'Нажмите на {npc:183339} в комнате с {npc:183337}, блокирующую путь к сокровищу.'
L['undulating_foliage_note'] = [[
Нажмите четыре {npc:185390}, чтобы активировать телепорт в центральной комнате Двора Катализатора.
• Снаружи за |cFFFFFD00Замком Двора Катализатора|r
• В юго-западной комнате двора
• В южной комнате с {npc:181652}
• В юго-восточной комнате двора
]]

L['provis_cache'] = 'Запертый тайник провиза'
L['provis_cache_note'] = [[
Выполните ежедневное задание {quest:64717} у {npc:177958} хотя бы один раз.
Используйте {item:187516} на кузницах во всей локации, чтобы собрать 15 {item:187728} и объединить их в {item:187787}. Эта сфера может содержать ключ.
]]
L['prying_eye_discovery'] = 'Находка любопытных'
L['prying_eye_discovery_note'] = 'Легче добраться, когда полеты разблокированы.'
L['pulp_covered_relic'] = 'Измазанная реликвия'
L['requisites_originator_note'] = [[
Машина, которая будет давать вам разные награды раз в неделю:

• {spell:366667} = Материалы для зачарования + золото
• {spell:366668} = Анима
• {spell:366669} = Шифры
• {spell:366670} = Частица творения + {item:189179} (шанс)
• {spell:366671} = Шифры/Частицы/Анима
• {spell:366672} = Шифровальное снаряжение
]]
L['rondure_cache'] = 'Сундук из округлого алькова'
L['rondure_cache_note'] = 'Интерактивная кузница в верхней части головоломки с прыжками внутри |cFFFFFD00Округлого Алькова|r.'
L['sandworn_chest'] = 'Пустынный сундук'
L['sandworn_chest_note'] = 'Соберите 5x {item:190198} с монстров в этом районе, чтобы создать {item:190197}.'
L['sand_piles'] = 'Скользящая куча песка'
L['sand_piles_note'] = [[
Все груды песка находятся в скрытой камере Дремлющий Альков. Каждая требует {item:189863}, которые можно найти в других сокровищницах в локации:

• Тайник Господства
• Хранилище Павших
• Украденный артефакт
• Украденный свиток
• Погруженный сундук
• Симфоническое хранилище
• Колыхающаяся листва
]]
L['torn_ethereal_drape'] = 'Рваная призрачная пелерина'
L['torn_ethereal_drape_note'] = 'Сначала берем руну |cFFFFFD00Ослепительный Альков|r. Далее активируем |cFFFFFD00Консоль автомов|r, и верхом на {npc:183565} дотянитесь до сокровища.'

L['cache_avian_nest'] = 'Гнездо авиана'
L['cache_cypher_bound'] = 'Защищенный шифром сундук'
L['cache_discarded_automa'] = 'Выброшенные обломки автома'
L['cache_forgotten_vault'] = 'Забытый склад сокровищ'
L['cache_mawsworn_supply'] = 'Сундук с припасами Верных Утробе'
L['cache_tarachnid_eggs'] = 'Яйцо тарахнидов'
L['cache_shrouded_cypher'] = 'Замаскированный тайник шифров'
L['cache_shrouded_cypher_note'] = [[
Эти тайники можно увидеть, только имея бафф {spell:361917} и надев соответствующий предмет Шифровального снаряжения:

|cFF00FF00Улучшает восприятие Покопока, позволяя ему обнаруживать дополнительные скрытые тайники.|r
]]

L['cache_cantaric'] = 'Тайник кантаты'
L['cache_fugueal'] = 'Тайник фуги'
L['cache_glissandian'] = 'Глиссандрийский тайник'
L['cache_mezzonic'] = 'Меццонийский тайник'
L['cache_toccatian'] = 'Токкатийский тайник'

L['schematic_treasure_note'] = '|cFFFF8C00Это сокровище не будет содержать схему протосинтеза, если вы еще не разблокировали синтез. Если вы уже получили это сокровище, вы найдете схему на земле поблизости.|r'
L['schematic_treasure_mount_note'] = '|cFFFF8C00Эта схема находится в ближайшем сокровище. Если сокровище открывалось до разблокировки синтеза средств передвижения, схему можно найти здесь.|r'
L['schematic_treasure_pet_note'] = '|cFFFF8C00Эта схема находится в ближайшем сокровище. Если сокровище открывается до разблокировки синтеза питомцев, схему можно найти здесь.|r'
L['schematic_bronze_helicid_note'] = 'Шанс получения из {item:190610} за задание {quest:65324}.'
L['schematic_ambystan_darter_note'] = 'Спрятан под водой рядом с {npc:185312}.'
L['schematic_bronzewing_vespoid_note'] = 'Внутри Приюта Отягощенных.'
L['schematic_buzz_note'] = 'Может быть получен из {npc:185265}.'
L['schematic_curious_crystalsniffer_note'] = 'Доступен в течение короткого времени после убийства {npc:184915} внутри |cFFFFFD00Святилища Господства|r.'
L['schematic_darkened_vombata_note'] = 'Внутри висящей клетки.'
L['schematic_deathrunner_note'] = 'Будет получено в ходе разблокировки {spell:366367}.'
L['schematic_desertwing_hunter_note'] = 'Расположен на вершине столба.'
L['schematic_fierce_scarabid_note'] = 'Спрятан под платформой, на которой стоит {npc:181870}.'
L['schematic_forged_spiteflyer_note'] = 'Торчит из скопления веспоидного улья.'
L['schematic_genesis_crawler_note'] = 'Находится над входом в Альков генезиса.'
L['schematic_goldplate_bufonid_note'] = 'Небольшой шанс получения с {npc:178803}.'
L['schematic_heartbond_lupine_note'] = 'Небольшой шанс получения из {npc:179939}.'
L['schematic_ineffable_skitterer_note'] = 'Умрите! Нет, правда... вы можете видеть {npc:185092} только в образе духа в |cFFFFFD00Низине Изгнанника|r.'
L['schematic_leaping_leporid_note'] = 'Расположен на вершине висящего дерева.'
L['schematic_mawdapted_raptora_note'] = 'Небольшой шанс получить из {npc:181412} в |cFFFFFD00Бесконечных Песках|r.'
L['schematic_microlicid_note'] = 'На самой нижней ветке висящего дерева, под листвой.'
L['schematic_omnipotential_core_note'] = 'Найдено в |cFFFFFD00Округлого Алькова|r. Схема спрятана за верхним краем арки на юго-западной стороне комнаты.'
L['schematic_prototickles_note'] = 'Прикреплен к висящей цепи внутри лозы над редким монстром {npc:180978}.'
L['schematic_prototype_fleetpod_note'] = 'Найден внутри |cFFFFFD00Camber Alcove|r. Взаимодействовать с {npc:184900} чтобы начать мини-игру. Проведите улитку через 5 колец, не касаясь {npc:185455} чтобы получить схему.'
L['schematic_raptora_swooper_note'] = 'Находится внутри Залов Ваяния.'
L['schematic_resonant_echo_note'] = 'Можно получить из сундуков |cFFFFFD00Кристаллизованное эхо первой песни|r.'
L['schematic_russet_bufonid_note'] = 'Можно получить из {item:187780} - награда идеала.'
L['schematic_scarlet_helicid_note'] = 'Расположен на вершине арочной конструкции.'
L['schematic_serenade_note'] = 'Расположена на цепи под висящей платформой в |cFFFFFD00Бессмертный очаг|r в рейдовой локации.'
L['schematic_shelly_note'] = 'Находится на задней стороне полки в Гроте Слов. Требуется способность телепортации, чтобы подняться на полку.'
L['schematic_stabilized_geomental_note'] = 'Выпадает с некоторым шансом из {npc:182169} в рейде |cFFFFFD00Гробница Предвечных|r.'
L['schematic_tarachnid_creeper_note'] = 'Внутри небольшого строения в |cFFFFFD00Бесконечных Песках|r. Требуется завершенная 5 глава кампании, чтобы убрать валуны.'
L['schematic_terror_jelly_note'] = 'На вершине квадратного пилона рядом с платформой.'
L['schematic_tunneling_vombata_note'] = 'Рядом с обломками в заблокированном туннеле внутри комплекса Локрийский эспер.'
L['schematic_vespoid_flutterer_note'] = 'Торчащий из кучи песка на уровне Сдвига Локуса в Пиках Отголосках.'
L['schematic_violent_poultrid_note'] = 'Можно получить из ежедневного задания {daily:65256}.'

L['concordance_excitable'] = 'Волнующий конкорданс'
L['concordance_mercurial'] = 'Оживленный конкорданс'
L['concordance_tranquil'] = 'Безмятежный конкорданс'
L['concordance_note'] = 'Прочитайте каждый конкорданс, чтобы разблокировать записи в |cFFFFFD00Своде конкордансов|r в |cFFFFFD00Низине изгнанника|r.'

L['echoed_jiro_note'] = 'Потратьте энергию у {npc:181059}, чтобы получить временные положительные эффекты. Требуется исследование |cFFFFFD00Созидайский|r у {npc:181397}'

L['bygone_elemental_note'] = '{npc:181221} может появиться вместо любого {npc:179007} в этом районе.'
L['dominated_irregular_note'] = '{npc:184819} может появиться вместо любого {npc:183184} в этом районе.'
L['gaiagantic_note'] = 'Появляется только тогда, когда {npc:177958} предлагает ежедневные задание {daily:64785}.'
L['misaligned_enforcer_note'] = 'Появляется здесь. Патрулирует территорию, а затем исчезает в конце своего маршрута.'
L['overcharged_vespoid_note'] = 'Может появиться как часть любой стаи в этом районе.'
L['runethief_xylora_note'] = 'Появляется скрытно в районе Приюта Пилигрима.'

local proto_area = 'Шанс найти в кузницах вокруг |cFFFFFD00%s|r.'
L['proto_material_zone_chance'] = 'Шанс найти в избранных кузницах по всей локации.'
L['anima_charged_yolk_note'] = 'Соберите {item:187728} из любой кузницы и объедините их в {item:187787}, который может содержать {item:187890}.'
L['energized_firmament_note'] = string.format(proto_area, 'Пики Отголосков')
L['honeycombed_lattice_note'] = string.format(proto_area, 'Гудящий утес')
L['incorporeal_sand_note'] = 'Шанс найти в кузницах в пустынных районах локации.'
L['pollinated_extraction_note'] = string.format(proto_area, 'Неукротимые заросли')
L['serene_pigment_note'] = 'В кузнице над входом в |cFFFFFD00Низину Изгнанника|r. Может потребоваться несколько попыток.'
L['volatile_precursor_note'] = 'Находится в маленькой кузнице наверху. Может потребоваться несколько попыток.'
L['wayward_essence_note'] = 'В висящей в воздухе кузнице позади |cFFFFFD00Низины Изгнанника|r. Встаньте в углубление, чтобы коснуться кузницы.'

L['patient_bufonid_note'] = 'Продвигайтесь по сюжету Зерет Мортис, пока {npc:180950} не предложит {quest:65727}. Выполняйте задания каждый день, чтобы выманить {npc:185798} из пруда.'
L['patient_bufonid_note_day1'] = 'Соберите 15x {item:190852} из Веспоидов в локации.'
L['patient_bufonid_note_day2'] = 'Купите 30x {item:172053} на аукционе.'
L['patient_bufonid_note_day3'] = 'Купите 200x {item:173202} на аукционе.'
L['patient_bufonid_note_day4'] = 'Купите 10x {item:173037} на аукционе.'
L['patient_bufonid_note_day5'] = 'Соберите 5x {item:187704} с монстров в локации.'
L['patient_bufonid_note_day6'] = 'Купите 5x {item:190880} у {npc:185748} рядом с Приютом Пилигрима.'
L['patient_bufonid_note_day7'] = 'Купите 1x {item:187171} у {npc:180114} в подземелье Тазавеш.'

L['lost_comb'] = 'Потерянные соты'
L['soulshape_penguin_note'] = 'Расположены на вершине летающей кузницы.'

L['coreless_automa'] = 'звери автомы'
L['coreless_automa_note'] = 'Экипировка с {spell:364480} даст |cFFFFFD00Покопоку|r возможность захватывать существ без расхода энергии и ускорит получение этого достижения.'
L['coreless_automa_warning'] = '|cFFFF8C00Подождите, пока автомата действительно начнет следовать за вами, прежде чем отозвать Покопока, чтобы избежать ошибки, требующей повторного входа в игру.|r'

L['olea_manu'] = 'Продает предметы коллекционирования и рецепты в обмен на {currency:1979}.'

L['venaris_fate_sublabel'] = 'Расположен в |cFFFFFF00Катализаторе творения|r'
L['venaris_fate_note'] = [[
1. Поговорите с проекцией {npc:162804} в |cFFFFFF00убежище Ве'нари|r в |cFFFFFF00Утробе|r.
2. Если вы видите не проекцию, требуется пройти дополнительные сюжетные задания в 9.2.
3. Взаимодействуйте с трупом {npc:162804} возле |cFFFFFF00Катализатора творения|r в |cFFFFFF00Зерет Мортис|r.
4. Через 5 дней вы получите письмо по почте от {npc:162804} вместе с {item:192485}.
]]

L['options_icons_code_creature'] = '{achievement:15211}'
L['options_icons_code_creature_desc'] = 'Показать местонахождение существ для достижения {achievement:15211}.'
L['options_icons_concordances'] = 'Свод конкордансов'
L['options_icons_concordances_desc'] = 'Отображение местоположения консолей свода конкордансов.'
L['options_icons_echoed_jiros'] = 'Вторящий Джиро'
L['options_icons_echoed_jiros_desc'] = 'Показать места, где |cFFFFFFD0Вторящие Джиро|r предлагают баффы.'
L['options_icons_exile_tales'] = '{achievement:15509}'
L['options_icons_exile_tales_desc'] = 'Отображение истории локации для достижения {achievement:15509}.'
L['options_icons_proto_materials'] = '{achievement:15229}'
L['options_icons_proto_materials_desc'] = 'Отображение местоположений редких материалов прототипов для достижения {achievement:15229}.'
L['options_icons_protoform_schematics'] = 'Схемы протосинтеза'
L['options_icons_protoform_schematics_desc'] = 'Отображать расположение схем на маунтов и питомцев.'
L['options_icons_puzzle_caches'] = 'Тайники с головоломками'
L['options_icons_puzzle_caches_desc'] = 'Показать возможные местоположения тайников с головоломками.'
L['options_icons_zereth_caches'] = 'Защищенные шифром тайники'
L['options_icons_zereth_caches_desc'] = 'Отображение возможных местоположений защищенных шифром тайников.'
L['options_icons_shrouded_cyphers'] = 'Замаскированный тайник шифров'
L['options_icons_shrouded_cyphers_desc'] = 'Показать возможные места для замаскированных тайников шифров.'
L['options_icons_mawsworn_supply_cache'] = 'Тайник Верных Утробе'
L['options_icons_mawsworn_supply_cache_desc'] = 'Показать возможные места для тайников Верных Утробе.'
L['options_icons_coreless_automa'] = '{achievement:15542}'
L['options_icons_coreless_automa_desc'] = 'Расположение пустых автоматов для достижения {achievement:15542}.'

L['ponderers_portal_label'] = nil
L['ponderers_portal_note'] = nil
